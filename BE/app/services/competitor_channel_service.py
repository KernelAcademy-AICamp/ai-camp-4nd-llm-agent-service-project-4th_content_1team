...
{{
  ...
  "comment_insights": {{
    ...
    async def analyze_reference_video(
        ...
        """
        참고 영상 AI 분석 (CompetitorRecentVideo 없이 동작).
        자막 + 댓글을 가져와 LLM으로 장단점, 시청자 반응, 적용 포인트 분석.
        """
        # 1. 자막 가져오기
        results = await SubtitleService.fetch_subtitles(
            video_ids=[youtube_video_id],
        ...
        caption_result = results[0] if results else {}
        tracks = caption_result.get("tracks", [])
        cue_count = sum(len(t.get("cues", [])) for t in tracks)
        ...
        if cue_count == 0:
            raise HTTPException(
                status_code=400,
                detail="자막이 없는 영상은 AI 분석을 할 수 없습니다.",
            )
        ...
        # 2. 댓글 가져오기
        comments_data = await CompetitorChannelService._fetch_youtube_comments(
            youtube_video_id, max_results=10
        )
        comments_context = ""
        if comments_data:
            comment_lines = []
            sorted_comments = sorted(
                comments_data, key=lambda x: -(x.get("likes", 0) or 0)
            )[:10]
            for c in sorted_comments:
                likes_str = f"(좋아요 {c.get('likes', 0)})" if c.get("likes") else ""
                comment_lines.append(f"- {c.get('text', '')} {likes_str}")
            comments_text = "\n".join(comment_lines)
            comments_context = f"""
[시청자 댓글 (좋아요 순 상위 {len(sorted_comments)}개)]
{comments_text}
"""
        ...
        try:
            ...
            if my_channel:
                persona_result = await db.execute(
                    select(ChannelPersona).where(
                        ChannelPersona.channel_id == my_channel.channel_id
                    )
                )
                persona = persona_result.scalar_one_or_none()
                if persona:
  ...
  "comment_insights": {{
    ...
    "needs": ["시청자 니즈1", "시청자 니즈2", ...]
        ...
        try:
            ...
            logger.error(f"참고 영상 LLM 분석 파싱 실패: {e}")
            raise HTTPException(
                status_code=500, detail="영상 분석 중 오류가 발생했습니다."
            )

        strengths = parsed.get("strengths", [])
        weaknesses = parsed.get("weaknesses", [])
        applicable_points = parsed.get("applicable_points", [])
        comment_insights = parsed.get("comment_insights", {
            "reactions": [],
            "needs": [],
        })
        analyzed_at = datetime.utcnow()

        logger.info(f"참고 영상 분석 완료: {youtube_video_id}")
