...
export function TopicDetailSidebar({ topic, onClose }: TopicDetailSidebarProps) {
  const navigate = useNavigate()
  const isGrowthType = topic.badge === "성공 방정식"

  const handleCreateScript = () => {
    navigate(`/script/edit?topic=${encodeURIComponent(topic.title)}`)
  }
    ...
    <div className="fixed right-0 top-0 h-full w-[400px] bg-card border-l border-border shadow-lg z-50 overflow-y-auto">
      ...
      <div className="p-6 space-y-6">
        <Badge
          ...
          className={cn(
            "text-sm font-medium",
            isGrowthType ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"
          )}
        >
          {topic.badge}
        </Badge>
        ...
        <Button onClick={handleCreateScript} className="w-full bg-primary hover:bg-primary/90" size="lg">
          이 주제로 스크립트 작성
        </Button>
      </div>
    </div>
