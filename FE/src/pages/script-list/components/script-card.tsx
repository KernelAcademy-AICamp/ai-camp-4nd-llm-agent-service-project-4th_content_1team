...
interface ScriptCardProps {
  ...
  created_at: string
  thumbnail?: string
  is_completed?: boolean // 완료 여부 (작성 완료 버튼 클릭 시 true)
  onClick?: () => void
}
...
export function ScriptCard({ 
  ...
  const getImageSource = () => {
    ...
    return "/images/script-dummy.png" // 기본 더미 이미지
  }
      ...
      <div className="relative h-[180px] w-full bg-muted">
        ...
        {!is_completed && (
          <div className="absolute top-3 right-3">
            <Badge 
              className="bg-gray-500/90 text-white hover:bg-gray-500 font-semibold text-sm px-3 py-1"
            >
              작성 중
            </Badge>
          </div>
