...
1. **Step 3-4 순서 변경**: 채널 방향성 → 지향 유튜버
2. **지향 유튜버**: 필수 → 선택 사항 (0개 가능)
3. **키워드 소스**: 카테고리 기반 추천
4. **연령대 UI**: Slider → Button
5. **성별 옵션**: "무관" 추가
6. **지향 유튜버 입력**: URL → 채널 검색 기능
...
interface StepTargetAudienceProps {
  onNext: (data: { gender: "male" | "female", ageGroup: number }) => void
  onPrev: () => void
  initialData?: { gender?: "male" | "female", ageGroup?: number }
}
```
...
### Step 3: 채널 방향성 (step-channel-concept.tsx)

#### 목적
롤모델 유튜버를 선택하여 경쟁 분석 및 벤치마킹 대상으로 활용
...
### Step 4: 지향 유튜버 (step-benchmark-channels.tsx) - 선택 사항
...
- 선택: 최소 1개, 최대 5개
- UI: Chip 선택 (다중)
- 소스: **Phase 2** - 지향 유튜버 분석 결과 (나중에 구현)
...
**표시 내용**: explore 페이지의 4가지 유형 카드 (2x2 그리드)
...
```tsx
// Step 3 완료 후
const keywords = await extractKeywordsFromBenchmarks(selectedChannels)
// → Step 4로 전달
<StepChannelConcept suggestedKeywords={keywords} />
```
